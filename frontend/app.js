...
const CARD_STATUS_KEYS = { active: 'dynamic.status_active', frozen: 'dynamic.status_frozen', cancelled: 'dynamic.status_cancelled' };
let loadedCards = [];
    ...
    list.innerHTML = cards.map(card => {
        ...
        <div class="credit-card virtual ${isCancelled ? 'cancelled' : ''}">
            ...
            <div class="card-footer">
                ...
                ${isCancelled
                    ? `<div class="status-indicator cancelled">${t('dynamic.status_cancelled', 'Cancelled')}</div>`
                    : `<button class="status-toggle-btn ${card.status}" data-card-id="${card.id}" data-action="${card.status === 'frozen' ? 'unfreeze' : 'freeze'}" title="${t(card.status === 'frozen' ? 'dynamic.status_frozen' : 'dynamic.status_active', card.status === 'frozen' ? 'Frozen' : 'Active')}">
                        <i data-lucide="${card.status === 'frozen' ? 'snowflake' : 'shield-check'}"></i>
                        <span>${t(CARD_STATUS_KEYS[card.status], card.status)}</span>
                    </button>`
                }
            </div>
            ${!isCancelled ? `
        ...
        btn.addEventListener('click', async () => {
            ...
            if (revealing) {
                ...
                if (faceEnrolled) {
                    btn.disabled = true;
                    const token = await requestFaceConfirmationToken(
                        t('cards.reveal_identity_hint', 'Verify your identity with the camera to see the full card number and CVV.')
                    );
                    btn.disabled = false;
                    if (!token) return;
                }
