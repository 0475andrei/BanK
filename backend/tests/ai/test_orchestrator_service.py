...
def test_route_always_returns_the_banking_agent(context):
    ...
    agent = BankingAgent(provider, build_banking_tools())
...
def test_dispatch_routes_and_runs_with_the_context(context):
    provider = MockProvider([ModelResponse(text="routed")])
    orchestrator = Orchestrator([BankingAgent(provider, build_banking_tools())])

    reply = orchestrator.dispatch([Message(role="user", content="hi")], "hi", context)

    assert reply == "routed"


def test_service_handle_message_end_to_end_with_tool_call(context):
    ...
    reply, history = service.handle_message([], "what's my balance?", context)
...
def test_service_reads_the_context_account_end_to_end(context):
    ...
    service.handle_message([], "balance?", context)
...
def test_service_threads_history_across_turns(context):
    service, provider = _service(
        [ModelResponse(text="first"), ModelResponse(text="second")]
    )

    _, history = service.handle_message([], "one", context)
    reply, history = service.handle_message(history, "two", context)
...
def test_service_does_not_mutate_the_history_it_was_given(context):
    service, _ = _service([ModelResponse(text="hi")])
    history: list[Message] = []

    service.handle_message(history, "hello", context)

    assert history == []


def test_handle_message_requires_a_context():
    """Identity has no default: omitting it is an error, not a silent fallback."""
    service, _ = _service([ModelResponse(text="hi")])

    with pytest.raises(TypeError):
        service.handle_message([], "hello")  # type: ignore[call-arg]
...
def test_orchestrator_rejects_duplicate_agent_names():
    ...
    with pytest.raises(ValueError):
        orchestrator.register(BankingAgent(provider, build_banking_tools()))
